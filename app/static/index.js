...
function win() {
  if ($(".type1.type4").length === target) {   //Determine whether to clear the game
    var scores = 0;
    if (level = 1) {        //According to different levels, and the number of completed steps to give players different points
      if (movetimes < 60) {
        scores1 = 40;
    ...
    $.ajax({   /*record database and render the movetimes in sobokan page*/
      url: "/index",
      type: "POST",
      data: {
          "scores1": scores1 ,
...
function toast(step, score) {
  setTimeout(() => {
    ...
    $(".mask .score-text").html(score)
    $(".mask").fadeIn();

  }, 200)
}

$(".mask .reload").click(function () {   /*when game done, reload popup window*/
  location.reload();
})

$(".mask .next").click(function () {   /* functions in popup window*/
  leaveNum++;
  if (leaveNum > 3) {
    leaveNum = 1;
  }
...
$("#open_btn").click(function () {   /* introduction of popup window*/
  $("#intro").fadeIn()
})
